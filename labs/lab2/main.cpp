...
void write_int(IFile &file, int n) {
    if (n < 0) {
        // 1. Сдвинуться на известное кол-во байт от начала `file`
        // 2. Перейти по этому адресу (попадаем на таблицу)
        // 3. Сдвинуться на адрес нужной функции (попадаем на адрес функции)
        // 4. Вызвать эту функцию
        file.write("-", 1);
        n = -n;
    }
...
int main() {
	...
	//Освобождение буфера
	for (int i = 0; i < 3; i++) {
	    // TODO: либо добавить flush в деструктор, либо сделать виртуальный метод (в
      //       IFile), который бы делал flush; второй вариант предпочтительнее
