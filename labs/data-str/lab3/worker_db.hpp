...
class WorkerDb {
    private:
	// TODO: какие операции нужны будут для обеспечения динамического
	// размера таблицы (увеличения кол-ва бакетов)

	//Нужно будет добавить метод, вычисляющий отношение кол-ва элементов к кол-ву корзин. Если намер это соотношение превышает 0.9, то нужно увеличивать корзину. Так же нужно добавить метод для увеличения корзины. Сейчас ее размер у меня статический, это тоже нужно будет исправить. Функция будет создавать новую таблицу, в два раза больше предыдущей, и переносить тужа все элементы.
	...
	//Метод, которые ищет сотрудника, но не добавляет мусор в случае, если его нет
	WorkerData* find(const char* last_name);
